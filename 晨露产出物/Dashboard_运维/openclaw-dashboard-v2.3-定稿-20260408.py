...
def get_output_files(bot_id):
    ...
    for root, dirs, filenames in os.walk(output_dir):
        ...
        for filename in filenames:
            ...
            # 跳过后缀文件和记录文件
            skip_files = ['版本记录', 'readme', 'README', 'README.md', 'readme.md', '版本记录.md']
            if any(filename.lower() == s.lower() or filename.lower().startswith(s.lower()) for s in skip_files):
                continue
...
class DashboardHandler(http.server.BaseHTTPRequestHandler):
    ...
    def generate_dashboard(self):
        ...
        # 任务列表 - 每个 Bot 显示2个最近任务
        tasks_html = ''
        # 按 Bot 分组，每个 Bot 只取2个最近任务
        ...
        for task in sorted_tasks:
            bot_id = task['bot']
            if bot_id not in bot_task_lists:
                bot_task_lists[bot_id] = []
            if len(bot_task_lists[bot_id]) < 2:  # 每个 Bot 最多2个
                bot_task_lists[bot_id].append(task)
        
        # 显示所有收集到的任务
        for bot_id, tasks in bot_task_lists.items():
            for task in tasks:
