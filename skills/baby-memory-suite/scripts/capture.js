...
function writeCrossAgentLog(content) {
  try {
    ...
    const importantKeywords = ['完成', '部署', '修复', '错误', '教训', '关键', '帮', '决策'];
    ...
    // 新格式：解析内容，提取执行者、受益者、动作
    let formattedEntry;
    
    // 如果内容已经有 [执行者/角色] 格式，直接记录
    if (content.includes('[') && content.includes(']')) {
      // 已有明确格式：[执行者/角色] 帮 受益者: 动作
      formattedEntry = `- ${today} ${time} ${content.substring(0, 100)}${content.length > 100 ? '...' : ''}\n`;
    } else {
      // 旧格式：转换为新格式
      // 尝试识别"帮"关键词
      if (content.includes('帮')) {
        formattedEntry = `- ${today} ${time} [${BOT_NAME}] ${content.substring(0, 100)}${content.length > 100 ? '...' : ''}\n`;
      } else {
        // 自己的记录
        formattedEntry = `- ${today} ${time} [${BOT_NAME}]: ${content.substring(0, 100)}${content.length > 100 ? '...' : ''}\n`;
      }
    ...
    fs.appendFileSync(PATHS.sharedLog, formattedEntry);
    log('INFO', '已记录到跨Bot日志');
    return true;
  } catch (error) {
