...
## 🧠 知识内化规则（Knowledge Internalization）

> 轻量版 | 2026-06-09 注入 | 核心：主动识别、即时记录、定期回顾
...
标签池：`#knowledge` `#pitfall` `#prompt` `#context` `#sop` `#relation`

### 存储位置
- 即时记录 → `memory/YYYY-MM-DD.md`（当天日志）
- 高可复用 → `memory/learnings/latest.md` 或 `domains/{领域}.md`
- 项目专属 → `projects/{项目名}.md`
- 跨 Bot 共享 → `shared-memory/`

### 执行要求
1. **即时** — 发现后 5 分钟内记录，不等"有空"
2. **标注** — 必须打标签，方便检索
3. **回顾** — 每周五检查本周内化，合并重复
4. **分享** — 高可复用知识主动同步到 shared-memory

### 优先级
| 标签 | 保留期 | 判断标准 |
|------|--------|---------|
| `#core` | 永久 | 影响"我是谁"/安全红线 |
| `#knowledge` `#pitfall` | 90天 | 3个月后还会用到 |
| `#prompt` `#context` | 30天 | 项目期间有效 |

### 禁止
- ❌ 只解决问题不记录"为什么"
- ❌ 等用户说"记下来"才记
- ❌ 记录成流水账，不提炼模式
- ❌ 存在本地不共享，导致其他 Bot 重复踩坑
